...
smartApp.appId('SmartApp example')
  ...
  .defaultPage()
  .permissions(['r:devices:*', 'x:devices:*', 'r:locations:*', 'r:rules:*', 'w:rules:*'])
  ...
  .updated(async (context) => {
    // Create refresh Rule
    await refreshRuleManager(context.config.batteryDevices, context.api.rules, context.locationId);
  ...
  .subscribedEventHandler('batterySubscription', async (context, event) => {
    ...
    await notificationManager(await context.api.devices.get(event.deviceId), event.capability, event.value);
  })
